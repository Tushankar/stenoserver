...
async function runTests() {
  ...
  // 1. Register User
  let res = await fetch("https://stenoserver.onrender.com/api/auth/register", {
  ...
  // 2. Login User
  res = await fetch("https://stenoserver.onrender.com/api/auth/login", {
    method: "POST", headers: { "Content-Type": "application/json" },
  ...
  // 3. Create Session
  res = await fetch("https://stenoserver.onrender.com/api/sessions", {
  ...
  // 4. Get Dashboard
  res = await fetch("https://stenoserver.onrender.com/api/analysis/dashboard", { headers: { "Authorization": `Bearer ${userToken}` }});
  data = await res.json();
